    ...
    public partial class NotificationViewModel : ViewModelBase
    {
        ...
        private void OnNotificationReceived(string message)
        {
            ...
            if (isRegistration)
            {
                ...
                WeakReferenceMessenger.Default.Send(new ViewModels.Messages.EntityUpdatedMessage("User", "Create", Guid.Empty));
            }
            else
            {
            ...
            try 
            {
                ...
                if (start > -1 && end > start)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        // Notify system
                        WeakReferenceMessenger.Default.Send(new ViewModels.Messages.EntityUpdatedMessage("User", "Update", user.Id));
                    }
                }
            }
            catch (Exception ex)
